...
/// builder.add(123).add("abc".to_string());
...
/// assert_eq!(*world.get::<&String>(f).unwrap(), "abc");
/// ```
...
struct Common<M> {
    ...
    indices: StableTypeIdMap<usize>,
}

impl<M> Common<M> {
    fn has<T: Component>(&self) -> bool {
        self.indices.contains_key(&T::STABLE_TYPE_ID)
    }

    fn get<'a, T: ComponentRefShared<'a>>(&'a self) -> Option<T> {
        let index = self.indices.get(&T::Component::STABLE_TYPE_ID)?;
